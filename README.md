...
└── sea_nav/                 # SEA-Nav mode (--data default)
    ...
    └── nav_model/model.onnx
```
> If the archive was zipped on macOS you may see `._*` sidecar files; they are harmless and can be removed with `find ~/Data/onboard_data/onnx_models -name '._*' -delete`.
