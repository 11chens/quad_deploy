...
class LocomotionAgent(BaseAgent):
    ...
    def infer_loco(self):
        self._vxy_pred = self.encoder_vel.run(
            None, {self.encoder_vel.get_inputs()[0].name: self.obs_hist.buffer.reshape(-1)}
        )[0]
        ...
        actions = self.policy_loco.run(None, {self.policy_loco.get_inputs()[0].name: self._actor_input})[0]
        return actions
