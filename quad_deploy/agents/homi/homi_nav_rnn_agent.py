...
class HomiNavRnnAgent(BaseRLAgent):
    def __init__(self, cfg=HomiNavAgentCfg, *args, **kwargs):
        ...
        # Initialize hidden states (h_in for GRU)
    ...
    def load_model(self):
        ...
        # Expected inputs: ['obs', 'h_in']
        # Expected outputs: ['action', 'h_out']

    def infer(self):
        # Concatenate observation input
        _actor_input = self.obs_buf.astype(np.float32).reshape(1, -1)

        # Run inference
        # Inputs: {input_name: obs, h_in_name: h}
        inputs = {
    ...
    def last_action(self):
        return self.action_before_clip
